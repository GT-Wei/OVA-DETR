        ...
        for metric in self.metrics:
            ...
            if metric == 'proposal':
                ...
                if self.classwise:  # Compute per-category AP
                    ...
                    # 获取Recall的数据,数据结构应该类似于precision
                    recalls = coco_eval.eval['recall']
                    # recall: (iou, recall, cls, area range, max dets),假设其结构与precision相同
                    ...
                    for idx, cat_id in enumerate(self.cat_ids):
                        ...
                        recall = recalls[0, idx, 0, 0]  # 取出对应类别在IOU=0.5时的所有召回率
                        recall = recall[recall > -1]  # 只考虑有效值
                        if recall.size:
                            avg_recall = np.mean(recall)
                        else:
                            avg_recall = float('nan')
                        
                        t.append(f'{round(avg_recall, 3)}')  # 添加到结果列表
                    ...
                    results_2d_list = list(itertools.zip_longest(*[
                        results_flatten[i::num_columns]
                        for i in range(num_columns)
                    ]))

                    # Calculate averages for each numeric column, starting from the second column
                    column_averages = []
                    for col_index in range(1, num_columns):  # Skip the first column (category names)
                        numeric_values = [float(row[col_index]) for row in results_2d_list if row[col_index] is not None]
                        numeric_values = [0.0 if math.isnan(value) else value for value in numeric_values]
                        if numeric_values:
                            average = sum(numeric_values) / len(numeric_values)
                            column_averages.append(round(average, 3))
                        else:
                            column_averages.append('NaN')

                    # Add the averages row to the results
                    averages_row = ['Averages'] + column_averages
                    
                    table_data = [headers]
                    # table_data += [result for result in results_2d]
                    table_data += results_2d_list
                    table_data += [averages_row]
